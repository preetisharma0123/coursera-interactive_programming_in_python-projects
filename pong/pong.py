...
ball_vel=[random.randrange(120, 240)*0.01,random.randrange(60, 180)*0.01]
paddle1_vel= 0
paddle2_vel = 0
...
def new_game():
    global paddle1_pos, paddle2_pos, paddle1_vel, paddle2_vel,paddle1_vel,paddle2_vel  # these are numbers
...
def draw(canvas):
    ...
    # update paddle's vertical position, keep paddle on the screen----8
    ...
    if (ball_pos[1] >= paddle1_pos-HALF_PAD_HEIGHT) and (ball_pos[1] <= (paddle1_pos+HALF_PAD_HEIGHT ))and (ball_pos[0]<=PAD_WIDTH+BALL_RADIUS):
        ball_vel[0]= - (ball_vel[0]+ball_vel[0]*.1)
        ball_vel[1]=   (ball_vel[1]+ball_vel[1]*.1)
    elif ball_pos[0]<=PAD_WIDTH+BALL_RADIUS:  
        spawn_ball(RIGHT)
        score2 +=1
    if (ball_pos[1] >= paddle2_pos-HALF_PAD_HEIGHT) and (ball_pos[1] <= paddle2_pos+HALF_PAD_HEIGHT) and (ball_pos[0]>=(WIDTH - PAD_WIDTH - BALL_RADIUS)):
        ball_vel[0]= - (ball_vel[0]+ball_vel[0]*.1)
        ball_vel[1]=  (ball_vel[1]+ball_vel[1]*.1)
